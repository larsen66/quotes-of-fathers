...
* **TR-1.1:** В качестве бэкенда используется **Firebase**.
* **TR-1.2:** Хранилище данных — **Firestore**.
* **TR-1.3:** Хранилище медиа — **Firebase Storage**.
* **TR-1.4:** Аутентификация администраторов — **Firebase Auth**.
* **TR-1.5:** Серверная логика (при необходимости) — **Firebase Cloud Functions**.

### TR-2. Админ-панель

* **TR-2.1:** Админ-панель реализуется как веб-приложение на **React** (с TypeScript).
* **TR-2.2:** Админ-панель использует **Firebase Admin SDK** для работы с Firestore и Firebase Storage.
* **TR-2.3:** Хостинг админ-панели осуществляется на **Firebase Hosting** (бесплатный тарифный план).
* **TR-2.4:** Аутентификация администраторов через **Firebase Auth**.
* **TR-2.5:** CRUD-операции для отцов и цитат реализуются через Firebase Admin SDK.
